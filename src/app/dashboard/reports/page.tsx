...
export default function ReportsPage() {
  ...
  // Statistik untuk laporan siswa
  const renderSiswaStats = () => {
    const siswaData = data as SiswaReportData;
    return (
      <div className="grid grid-cols-1 sm:grid-cols-1 gap-4 mb-6">
        <div className="bg-emerald-50 rounded-2xl p-6 border border-emerald-100">
          <p className="text-sm font-medium text-emerald-700">Total Siswa</p>
          <p className="text-2xl font-bold text-emerald-800">
            {siswaData?.totalSiswa ?? 0}
          </p>
        </div>
      </div>
    );
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            {bookData.books.slice((page - 1) * 10, page * 10).map((book, index) => (
        ...
        <PaginationControls
          currentPage={page}
          totalPages={Math.ceil(bookData.books.length / 10) || 1}
          totalItems={bookData.books.length}
          itemsPerPage={10}
    ...
    if (!siswaData?.siswa || siswaData.siswa.length === 0) {
      return (
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="mt-2 text-sm">Tidak ada data siswa</p>
        </div>
      );
    }
    ...
    return (
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            {siswaData.siswa.slice((page - 1) * 10, page * 10).map((s, index) => (
              <tr key={index} className="h-16 hover:bg-gray-50 transition">
                <td className="px-6 py-3 text-sm font-semibold text-gray-900">
                  {s.Nama}
                </td>
        ...
        <PaginationControls
          currentPage={page}
          totalPages={Math.ceil(siswaData.siswa.length / 10) || 1}
          totalItems={siswaData.siswa.length}
          itemsPerPage={10}
          onPageChange={setPage}
        />
      </div>
    ...
    return (
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            {borrowingData.borrowings.slice((page - 1) * 10, page * 10).map((borrowing, index) => (
              <tr key={index} className="h-16 hover:bg-gray-50 transition">
        ...
        <PaginationControls
          currentPage={page}
          totalPages={Math.ceil(borrowingData.borrowings.length / 10) || 1}
          totalItems={borrowingData.borrowings.length}
          itemsPerPage={10}
          onPageChange={setPage}
        />
      </div>
  ...
  const renderClusteringTable = () => {
    const clusteringData = data as any;
    if (!clusteringData?.siswa || clusteringData.siswa.length === 0) {
      return (
        <div className="text-center py-12 text-gray-500">
          <Users className="mx-auto h-12 w-12 text-gray-300" />
          <p className="mt-2 text-sm">Belum ada data hasil clustering.</p>
        </div>
      );
    }
    ...
    return (
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            {clusteringData.siswa.slice((page - 1) * 10, page * 10).map((s: any, index: number) => (
              <tr key={index} className="h-16 hover:bg-gray-50 transition">
                <td className="px-6 py-3 text-sm font-semibold text-gray-900">
                  {s.Nama}
                </td>
                ...
                <td className="px-6 py-3 text-sm text-center font-semibold text-emerald-700">
                  {s.Peminjaman}
                </td>
                <td className="px-6 py-3 text-sm text-center font-semibold text-blue-700">
                  {s.Pengembalian}
                </td>
                <td className="px-6 py-3 text-sm text-center font-semibold text-purple-700">
                  {s["E-Book"]}
                </td>
                <td className="px-6 py-3 text-center">
                  <span
        ...
        <PaginationControls
          currentPage={page}
          totalPages={Math.ceil(clusteringData.siswa.length / 10) || 1}
          totalItems={clusteringData.siswa.length}
          itemsPerPage={10}
          onPageChange={setPage}
        />
      </div>
              ...
              <div className="space-y-2">
                ...
                <Select value={reportType} onValueChange={setReportType}>
                  ...
                  <SelectContent>
                    ...
                    {isAdmin && (
                      <>
                        <SelectItem value="members">Laporan Siswa</SelectItem>
                        <SelectItem value="clustering">
                          Laporan Clustering (K-Means)
                        </SelectItem>
                      </>
                    )}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg">
              ...
              {reportType === "members" && "Laporan Siswa"}
