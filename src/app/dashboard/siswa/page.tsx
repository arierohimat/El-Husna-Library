...
import { User, Bookmark, AlertCircle, Calendar, TrendingUp, BookOpen } from "lucide-react";
...
export default async function SiswaDashboard() {
  ...
  // Get member's recent eBook accesses
  const recentEbookAccesses = await db.eBookAccess.findMany({
    where: {
      userId: session.userId,
    },
    include: {
  ...
  return (
    <DashboardLayout userRole={session.role} userName={session.name}>
      <div className="space-y-6">
        ...
        {/* E-Book Accesses Section */}
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              <BookOpen className="w-5 h-5 text-emerald-500" />
              E-Book yang Pernah Dibaca
            </CardTitle>
            <Link
          ...
          <CardContent>
            {recentEbookAccesses.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-sm text-gray-500">Belum ada E-Book yang dibaca.</p>
                <p className="text-xs text-gray-400 mt-1">Jelajahi koleksi digital perpustakaan untuk mulai membaca!</p>
              </div>
            ) : (
              <div className="grid gap-4 sm:grid-cols-2">
                {recentEbookAccesses.map((acc) => (
                  <div key={acc.id} className="p-4 rounded-xl border border-gray-100 bg-white shadow-sm flex items-center justify-between gap-3 hover:border-emerald-200 transition-all">
                    <div className="min-w-0 flex-1">
                      <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-semibold text-emerald-700">
                        {acc.ebook.category}
                      </span>
                      <p className="text-sm font-bold text-gray-900 truncate mt-1">{acc.ebook.title}</p>
                      <p className="text-xs text-gray-500 truncate">{acc.ebook.author}</p>
                      <p className="text-[10px] text-gray-400 mt-2">
                        Dibaca {acc.accessCount}x • Terakhir: {new Date(acc.lastReadAt).toLocaleDateString("id-ID")}
                      </p>
                    </div>
                    <Link
                      href={`/dashboard/ebooks/${acc.ebookId}`}
                      className="px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-semibold flex-shrink-0 flex items-center gap-1 shadow-sm"
                    >
